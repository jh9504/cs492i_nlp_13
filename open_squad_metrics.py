...
def get_final_text(pred_text, orig_text, do_lower_case, verbose_logging=False):
    ...
    if start_position == -1:
        if verbose_logging:
            ...
            logger.info("prediction text: '%s' /n original text: '%s'", pred_text, orig_text) ###
            '''
            1
        return orig_text
    ...
    if len(orig_ns_text) != len(tok_ns_text):
        if verbose_logging:
            ...
            logger.info("original text: '%s' /n token text: '%s'", orig_text, tok_text) ###
            '''
            1
        return orig_text
...
def select_best_predictions(all_nbest_json):
    ...
    print("INSIDE FUNCTION BUT NOT INSIDE FOR LOOP")
    logger.info("INSIDE FUNCTION BUT NOT INSIDE FOR LOOP")
    for qas_id, nbest_json in all_nbest_json.items():
        ...
        logger.info("\nqa_id_without_s", qa_id_without_s)
        logger.info("\ntext", text)
        logger.info("\nprob", prob)
